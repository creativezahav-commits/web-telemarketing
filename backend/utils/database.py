...
def init_db():
    conn = get_conn()
    cur  = conn.cursor()
    cur.execute("""CREATE TABLE IF NOT EXISTS akun (
        phone TEXT PRIMARY KEY, nama TEXT, username TEXT,
        status TEXT DEFAULT 'active',
        dibuat TEXT DEFAULT (datetime('now','localtime')))""")
    cur.execute("""CREATE TABLE IF NOT EXISTS grup (
        id INTEGER PRIMARY KEY, nama TEXT, username TEXT,
        tipe TEXT, jumlah_member INTEGER, link TEXT,
        status TEXT DEFAULT 'active',
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS antrian (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        phone TEXT NOT NULL, grup_id INTEGER NOT NULL,
        pesan TEXT NOT NULL, status TEXT DEFAULT 'menunggu',
        dibuat TEXT DEFAULT (datetime('now','localtime')), dikirim TEXT)""")
    cur.execute("""CREATE TABLE IF NOT EXISTS riwayat (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        phone TEXT NOT NULL, grup_id INTEGER NOT NULL,
