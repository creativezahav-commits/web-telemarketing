...
def simpan_akun(phone, nama, username):
    conn = get_conn()
    conn.execute("""INSERT INTO akun (phone, nama, username)
        VALUES (?, ?, ?) ON CONFLICT(phone) DO UPDATE SET
        nama=excluded.nama, username=excluded.username""",
...
def get_status_akun(phone):
    ...
    conn.close(); return row["status"] if row else "active"
...
def simpan_banyak_grup(daftar):
    conn = get_conn()
    for g in daftar:
        conn.execute("""INSERT INTO grup (id,nama,username,tipe,jumlah_member,link,status)
            VALUES (?,?,?,?,?,?,'active') ON CONFLICT(id) DO UPDATE SET
            nama=excluded.nama, username=excluded.username,
            tipe=excluded.tipe, jumlah_member=excluded.jumlah_member,
            link=excluded.link, diupdate=datetime('now','localtime')""",
...
def tambah_antrian(phone, grup_id, pesan):
    conn = get_conn()
    cur  = conn.execute("INSERT INTO antrian (phone,grup_id,pesan) VALUES (?,?,?)",
                        (phone, grup_id, pesan))
...
def get_antrian_menunggu():
    conn = get_conn()
    rows = conn.execute("SELECT * FROM antrian WHERE status='menunggu' ORDER BY id").fetchall()
    conn.close(); return [dict(r) for r in rows]

def update_status_antrian(item_id, status):
    conn = get_conn()
    conn.execute("UPDATE antrian SET status=?, dikirim=datetime('now','localtime') WHERE id=?",
                 (status, item_id))
    conn.commit(); conn.close()
...
def catat_riwayat(phone, grup_id, nama_grup, status, pesan_error=None):
    conn = get_conn()
    conn.execute("INSERT INTO riwayat (phone,grup_id,nama_grup,status,pesan_error) VALUES (?,?,?,?,?)",
...
def get_riwayat_hari_ini():
    ...
    rows = conn.execute("SELECT * FROM riwayat WHERE waktu LIKE ? ORDER BY id DESC",
                        (f"{hari}%",)).fetchall()
    conn.close(); return [dict(r) for r in rows]
...
def get_ringkasan_hari_ini():
    ...
    row  = conn.execute("""SELECT
        ...
        COUNT(*) as total FROM riwayat WHERE waktu LIKE ?""",
        (f"{hari}%",)).fetchone()
    conn.close()
    return {"berhasil":row["berhasil"] or 0,"gagal":row["gagal"] or 0,
            "skip":row["skip"] or 0,"total":row["total"] or 0}

def sudah_dikirim_hari_ini(grup_id):
    conn = get_conn()
    hari = datetime.now().strftime("%Y-%m-%d")
    row  = conn.execute("SELECT COUNT(*) as n FROM riwayat WHERE grup_id=? AND status='berhasil' AND waktu LIKE ?",
                        (grup_id, f"{hari}%")).fetchone()
    conn.close(); return row["n"] > 0

def hitung_kirim_hari_ini(phone):
    conn = get_conn()
    hari = datetime.now().strftime("%Y-%m-%d")
    row  = conn.execute("SELECT COUNT(*) as n FROM riwayat WHERE phone=? AND status='berhasil' AND waktu LIKE ?",
                        (phone, f"{hari}%")).fetchone()
    conn.close(); return row["n"] or 0
```
