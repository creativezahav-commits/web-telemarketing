...
def tandai_akun_restricted(phone: str, alasan: str = '') -> bool:
    """Tandai akun yang dibatasi Telegram (spam warning / restricted).
    Berbeda dengan banned: akun masih hidup tapi tidak boleh kirim/join bebas.
    Status diset 'restricted' dan auto_send dimatikan sementara.
    """
    conn = get_conn()
    cur = conn.execute(
        """UPDATE akun
           SET status='restricted',
               auto_send_enabled=0,
               last_error_code='restricted',
               last_error_message=%s
           WHERE phone=%s AND status NOT IN ('banned','restricted')""",
        (alasan[:200] if alasan else 'Akun dibatasi Telegram', phone)
    )
    updated = cur.rowcount > 0
    conn.commit()
    conn.close()
...
def simpan_draft(judul, isi):
    conn = get_conn()
    cur = conn.execute("INSERT INTO draft (judul, isi) VALUES (%s, %s) RETURNING id", (judul, isi))
    did = cur.lastrowid
    conn.commit()
...
def tambah_antrian(phone, grup_id, pesan):
    conn = get_conn()
    cur = conn.execute(
        "INSERT INTO antrian (phone, grup_id, pesan) VALUES (%s, %s, %s) RETURNING id",
        (phone, grup_id, pesan),
    )
    iid = cur.lastrowid
    conn.commit()
...
def get_riwayat_hari_ini(limit: int = 2000):
    """Ambil semua riwayat hari ini. Limit default 2000 agar statistik card akurat
    meski volume tinggi. Sebelumnya tidak ada limit eksplisit."""
    conn = get_conn()
    hari = datetime.now().strftime("%Y-%m-%d")
    rows = conn.execute(
        "SELECT * FROM riwayat WHERE waktu LIKE %s ORDER BY id DESC LIMIT %s",
        (hari + "%", int(limit)),
    ).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
def create_permission(group_id: int, permission_basis: str, approval_source: str | None, approved_by: str | None, approved_at: str | None, expires_at: str | None = None, notes: str | None = None, status: str = 'valid') -> int:
    conn = get_conn()
    # Cek apakah sudah ada permission untuk grup ini
    existing = conn.execute(
        "SELECT id FROM group_permission WHERE group_id=%s ORDER BY id DESC LIMIT 1",
        (group_id,)
    ).fetchone()
    if existing:
        # Sudah ada → update saja, jangan insert baru
        pid = int(existing['id'])
        conn.execute(
            """UPDATE group_permission SET
               permission_basis=%s, approval_source=%s, approved_by=%s,
               approved_at=%s, expires_at=%s, status=%s, notes=%s,
               updated_at=TO_CHAR(NOW(),'YYYY-MM-DD HH24:MI:SS')
               WHERE id=%s""",
            (permission_basis, approval_source, approved_by,
             approved_at, expires_at, status, notes, pid),
        )
    else:
        # Belum ada → insert baru
        cur = conn.execute(
            """INSERT INTO group_permission
               (group_id, permission_basis, approval_source, approved_by,
                approved_at, expires_at, status, notes)
    ...
    # Sync ke tabel grup
    conn.execute(
        "UPDATE grup SET permission_status=%s, permission_basis=%s, "
        "approved_by=%s, approved_at=%s, permission_expires_at=%s WHERE id=%s",
...
def create_assignment(group_id: int, account_id: str, assignment_type: str = 'sync_owner', status: str = 'assigned', priority_level: int = 100, assign_reason: str | None = None, assign_score_snapshot: str | None = None) -> int:
    ...
    cur = conn.execute(
        ...
        VALUES (%s, %s, %s, %s, %s, %s, %s, TO_CHAR(NOW(),'YYYY-MM-DD HH24:MI:SS')) RETURNING id
        """,
...
def create_automation_rule(name: str, rule_type: str, enabled: int = 1, priority: int = 100, condition_json: str | None = None, action_json: str | None = None, cooldown_seconds: int = 0, scope_json: str | None = None) -> int:
    conn = get_conn()
    cur = conn.execute(
        'INSERT INTO automation_rule (name, rule_type, enabled, priority, condition_json, action_json, cooldown_seconds, scope_json) VALUES (%s, %s, %s, %s, %s, %s, %s, %s) RETURNING id',
